    ...
    for(int i=0;i<SAT_NUM;i++)
    {
        ...
        if(i!=src_id-1 && next_hop[i]!=-1)
        {
            ...
            if(linkid != -1)
            {
				...
				args.src_id[i]=src_id;
				args.gw_id[i]=gw_id;
				args.dst_id[i]=dst_id;
				args.src_portid[i]=src_portid;
				args.gw_portid[i]=gw_portid;
				args. linkid[i]= linkid;
				RouteTask *rt=new RouteTask((void*)&args);
				route_pool.addTask(rt);
				LinkRouteTask *lrt=new LinkRouteTask((void*)&args);
